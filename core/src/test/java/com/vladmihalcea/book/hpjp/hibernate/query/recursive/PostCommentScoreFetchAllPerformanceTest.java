    ...
    protected List<PostCommentScore> postCommentScores(Long postId, int rank) {
        return doInJPA(entityManager -> {
            ...
            List<PostCommentVote> postCommentVotes = entityManager.createQuery(
                ...
                "inner join fetch pc.post p " +
                "where p.id = :postId", PostCommentVote.class)
            ...
            Map<Long, PostCommentScore> topPostCommentScoreMap = new TreeMap<>(Comparator.reverseOrder());

            for(PostCommentScore postCommentScore : roots) {
                topPostCommentScoreMap.put(postCommentScore.getTotalScore(), postCommentScore);
            }

            List<PostCommentScore> result = new ArrayList<>(topPostCommentScoreMap.values());
            if(result.size() > rank) {
                result = result.subList(0, rank);
            }
